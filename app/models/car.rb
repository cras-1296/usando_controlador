class Car < ApplicationRecord
  ...
  def store_in_trunk(amount)
    if current_trunk_usage + amount <= max_trunk_space
      update!(current_trunk_usage: current_trunk_usage + amount)
    else
      raise RuntimeError, 'Cannot store requested amount, total exceeds maximum'
    end
  end
